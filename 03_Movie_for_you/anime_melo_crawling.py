...
check_xpath = '//*[@id="contents"]/section/div[4]/div/div[2]/div/div[{}]/button'.format(5)

start_url = 'https://m.kinolights.com/discover/explore'
driver.get(start_url)
time.sleep(0.5)
button_anime = driver.find_element(By.XPATH, button_anime_xpath)
driver.execute_script('arguments[0].click()', button_anime)
time.sleep(0.5)
check_melo = driver.find_element(By.XPATH, check_xpath)
driver.execute_script('arguments[0].click()', check_melo)

scroll_to_bottom()

hrefs = []
titles = []
for i in range(100, 200): # 1~100, 100~200, ..., 900~1000
...
my_name = 'OHS'
df.to_csv('./data/reviews_{}2.csv'.format(my_name), index=False)