...
# 라벨 인코딩
with open('./models/last_models/encoder.pickle', 'rb') as f:
...
# 토크나이저 로드 및 시퀀스 변환
with open('./models/last_models/token_max_50.pickle', 'rb') as f:
    token = pickle.load(f)
...
for i in range(len(tokened_x)):
    tokened_x[i] = tokened_x[i][:50]
x_pad = pad_sequences(tokened_x, 50)
print(x_pad[:10])

# 모델 로드 및 예측
model = load_model('./models/last_models/news_section_classification_model_0.82.h5')
